...
export function useStudentLabels() {
  const [labels, setLabels] = useState<StudentLabel[]>([]);
  ...
  const resolveModuleId = async (): Promise<string | null> => {
    ...
    try {
      ...
      const found = (json.modules || []).find(
        (m: any) => (m.code || '').toLowerCase() === 'students',
      );
      ...
      console.error('Resolve students moduleId error:', error);
    }
  ...
  const fetchLabels = async (id?: string) => {
    ...
    try {
      ...
      console.error('Fetch student labels error:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const createLabel = async (name: string, color: string) => {
    ...
    try {
      ...
      console.error('Create student label error:', error);
    } finally {
      setLoading(false);
    }
  ...
  const deleteLabel = async (id: string) => {
    ...
    try {
      ...
      console.error('Delete student label error:', error);
    } finally {
      setLoading(false);
    }
